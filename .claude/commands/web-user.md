...
### 3단계: 작업 실행 (web-user 에이전트)

web-user 에이전트에 작업을 위임한다. 에이전트는 아래 규칙을 따른다:
- React 18+ (Vite) + TypeScript + Tailwind + shadcn/ui
- Zustand (클라이언트 상태) + TanStack Query (서버 상태)
- Axios 인터셉터로 JWT/에러 일괄 처리
- React Router v6, React Hook Form + Zod
- Framer Motion (룰렛 애니메이션), sonner (토스트)
- 도메인별 디렉토리 (roulette/, point/, product/, order/)
- 도메인별 API 모듈 (api/roulette.ts 등)
- useEffect로 API 호출 금지, TanStack Query 사용
- CSS 파일 생성 금지, Tailwind 유틸리티 클래스 사용
